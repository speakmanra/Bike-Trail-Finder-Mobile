...
async function printRunInstructionsAsync() {
  let user = await _xdl().UserManager.getCurrentUserAsync(); // If no user, we are offline and can't connect
  ...
  if (user) {
    ...
    (0, _log().default)(`${_chalk().default.underline('iOS devices')}: run ${_chalk().default.bold('exp send -s <your-email-address>')} in this project directory in another terminal window to send the URL to your device.`); // NOTE(brentvatne) Uncomment this when we update iOS client
    // log(
  ...
  (0, _log().default)(`If you already have the simulator installed, run ${_chalk().default.bold('exp ios')} or ${_chalk().default.bold('exp android')} in this project directory in another terminal window.`);
