...
async function installAsync(packages, options) {
  const {
    projectRoot,
    workflow
  } = await findProjectRootAsync(process.cwd());
...
async function findProjectRootAsync(base) {
  ...
  do {
    if (await _jsonFile().default.getAsync(_path().default.join(dir, 'app.json'), 'expo', null)) {
      return {
        projectRoot: dir,
        workflow: 'managed'
      };
    } else if (_fs().default.existsSync(_path().default.join(dir, 'package.json'))) {
      return {
        projectRoot: dir,
        workflow: 'bare'
      };
