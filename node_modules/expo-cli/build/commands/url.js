...
const logArtifactUrl = platform => async (projectDir, options) => {
  ...
  const res = await _xdl().Project.buildAsync(projectDir, {
    current: false,
    mode: 'status',
    ...(options.publicUrl ? {
  ...
  const url = _fp().default.compose(_fp().default.get(['artifacts', 'url']), _fp().default.head, _fp().default.filter(job => platform && job.platform === platform), _fp().default.getOr([], 'jobs'))(res);
...
async function action(projectDir, options) {
  ...
  if ((await _xdl().Project.currentStatus(projectDir)) !== 'running') {
    throw new (_CommandError().default)('NOT_RUNNING', `Project is not running. Please start it with \`${options.parent.name} start\`.`);
  }

  let url;

  if (options.web) {
    url = await _xdl().UrlUtils.constructWebAppUrlAsync(projectDir);
  } else {
    url = await _xdl().UrlUtils.constructManifestUrlAsync(projectDir);
  }
...
var _default = program => {
  program.command('url [project-dir]').alias('u').option('-w, --web', 'Return the URL of the web app').description('Displays the URL you can use to view your project in Expo').urlOpts().allowOffline().asyncActionProjectDir(action,
