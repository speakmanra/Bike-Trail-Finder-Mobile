...
const printHelp = () => {
  ...
  _log().default.nested(`Press ${b('?')} to show a list of all available commands.`);
};
...
const printUsage = async projectDir => {
 ...
 \u203A Press ${b`shift-d`} to ${openDevToolsAtStartup ? 'disable' : 'enable'} automatically opening ${u`D`}evTools at startup.
...
const printServerInfo = async projectDir => {
  ...
  async function handleKeypress(key) {
    switch (key) {
      ...
      case 'w':
        {
          clearConsole();
          (0, _log().default)('Trying to open the project in a web browser...');
      ...
      case 'R':
        {
          ...
          _xdl().Project.startAsync(projectDir, {
            reset
          });
