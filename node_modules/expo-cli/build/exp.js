...
function runAsync(programName) {
  try {
    ...
    const commandFiles = [].concat(_glob().default.sync('commands/*.js', {
      cwd: __dirname
    }), _glob().default.sync('commands/*/index.js', {
      cwd: __dirname
    }));
    commandFiles.sort().forEach(file => {
      ...
      if (typeof commandModule === 'function') {
        commandModule(_commander().default);
      } else if (typeof commandModule.default === 'function') {
        commandModule.default(_commander().default);
      } else {
        _log().default.error(`'${file}.js' is not a properly formatted command.`);
      }
    });
...
Run \`npm install -g ${_package().default.name}\` to get the latest version`), {
      borderColor: 'green',
      padding: 1
    }));
  }
